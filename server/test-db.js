...
const { initializeDatabase, queryOne, query } = require('./db/database');
...
async function test() {
  ...
  try {
    ...
    `);
    console.log('expiringBatches', expiringBatches);
    ...
    `);
    console.log('salesTrend', salesTrend);
    ...
    const expiryAlerts = await query(`
      ...
      LIMIT 5
    `);
    console.log('expiryAlerts', expiryAlerts);
