...
router.get('/profit-loss', verifyToken, async (req, res) => {
  try {
    ...
    const revenue = await queryOne(`
      ...
      FROM sales
      WHERE status = 'paid' AND sold_at::date >= ?::date AND sold_at::date <= ?::date
    `, [start, end]);
    ...
    const cogs = await queryOne(`
      ...
      WHERE s.status = 'paid' AND s.sold_at::date >= ?::date AND s.sold_at::date <= ?::date
    `, [start, end]);
    ...
    let dateFormat;
    if (group_by === 'monthly') {
      dateFormat = "TO_CHAR(sold_at::date, 'YYYY-MM')";
    } else if (group_by === 'weekly') {
      dateFormat = "TO_CHAR(sold_at::date, 'IYYY-\"W\"IW')";
    } else {
      dateFormat = "sold_at::date::text";
    }
  ...
  try {
    ...
    const expiring = await query(`
      SELECT pb.id, pb.batch_number, pb.expiry_date, pb.qty_on_hand, pb.purchase_price,
             ...
             (pb.expiry_date::date - CURRENT_DATE) as days_until_expiry
